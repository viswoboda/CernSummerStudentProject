...
TGeoManager *PandoraMonitoring::m_pTGeoManager = nullptr;
...
void PandoraMonitoring::InitializeEve(Char_t transparency)
{
    if (nullptr == m_pEveManager)
    {
        ...
        {
            // Build the detector outline in a geometry manager owned by PandoraMonitoring, kept separate from any
            // externally-created one. Under DD4hep-based reconstruction (e.g. k4GaudiPandora) gGeoManager already holds
            // the full, closed DD4hep geometry (named "default"). Two distinct pitfalls:
            //   1. Reusing that manager and calling SetTopVolume() on it does not draw our outline (observed: empty
            //      geometry view, no error). This regressed against an earlier ROOT; exact version/PR still TBD.
            //   2. The TGeoManager constructor deletes whatever geometry is currently registered in gGeoManager (this
            //      is long-standing ROOT behaviour). So we detach gGeoManager before constructing ours; otherwise
            //      DD4hep's geometry is freed and the dangling gGeoManager later crashes TEveGeoManagerHolder on paint.
            TGeoManager *const pPreviousGeoManager = gGeoManager;

            TGeoManager *pGeoManager = nullptr;
            if (nullptr != m_pTGeoManager)
            {
                pGeoManager = m_pTGeoManager;
                gGeoManager = pGeoManager; // make our manager current so newly created TGeo objects register with it
            }
            else
            {
                // The TGeoManager constructor deletes the currently-registered geometry (gGeoManager) and adopts itself.
                // Detach gGeoManager first so the existing manager (e.g. DD4hep's) is kept alive; we re-point to it below.
                gGeoManager = nullptr;
                pGeoManager = new TGeoManager("PandoraMonitoringGeometry", "PandoraMonitoring detector geometry"); // sets gGeoManager
               m_pTGeoManager = pGeoManager;
            }
            ...
            // Restore the previously-current geometry manager (e.g. DD4hep's) so the rest of the job is unaffected.
            // In standalone running there was none, so we keep our own manager current (as the original code did).
            if (nullptr != pPreviousGeoManager)
                gGeoManager = pPreviousGeoManager;
